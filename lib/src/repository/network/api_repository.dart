...
class DefaultAPIRepository implements APIRepository {
  ...
  Future<Map<String, dynamic>> request({required Endpoint endpoint}) async {
    ...
    final url = Uri.https(
      SharedConfiguration.network.isDebug
          ? SharedConfiguration.network.baseUrlDevelopment
          : SharedConfiguration.network.baseUrlProduction,
      endpoint.path,
  ...
  Map<String, dynamic> _handleResponse(Response response) {
    ...
    if (response.headers.containsKey('authorization')) {
      PreferencesUser().setToken = response.headers['authorization'] ?? '';
    } else if (decodedBody.containsKey('token')) {
      PreferencesUser().setToken = decodedBody['token'] ?? '';
    }
